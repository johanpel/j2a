...
auto cuDFBatteryParse(const BatteryParserWorkload& data) -> BatteryParserResult {
  BatteryParserResult result("cuDF", "read_json", false);
  result.timer.Start();

  cudf::io::json_reader_options read_options = cudf::io::json_reader_options::builder(cudf::io::source_info{data.bytes.data(), data.num_bytes});
